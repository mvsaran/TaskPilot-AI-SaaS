import { PrismaClient, UserRole, ProjectStatus, TaskStatus, Priority, SprintStatus, DocumentStatus, NotificationType } from '@prisma/client';
...
async function main() {
  ...
  const demoAccounts = [
    { email: 'admin@taskpilot.ai', fullName: 'Sarah Connor (Admin)', role: UserRole.ADMIN },
    { email: 'pm@taskpilot.ai', fullName: 'John Vance (Project Manager)', role: UserRole.PROJECT_MANAGER },
    { email: 'dev@taskpilot.ai', fullName: 'Alex Rivera (Senior Dev)', role: UserRole.DEVELOPER },
    { email: 'qa@taskpilot.ai', fullName: 'Maya Lin (Lead QA)', role: UserRole.QA_ENGINEER },
    { email: 'po@taskpilot.ai', fullName: 'David Chen (Product Owner)', role: UserRole.PRODUCT_OWNER },
    { email: 'viewer@taskpilot.ai', fullName: 'Emma Watson (Stakeholder Viewer)', role: UserRole.VIEWER },
  ];
  ...
  const rolesList: UserRole[] = [
    UserRole.DEVELOPER, UserRole.DEVELOPER, UserRole.DEVELOPER,
    UserRole.QA_ENGINEER, UserRole.PROJECT_MANAGER, UserRole.PRODUCT_OWNER, UserRole.VIEWER
  ];
  ...
  for (const user of allUsers) {
    ...
    teamMemberData.push({
      ...
      role: user.role === UserRole.PROJECT_MANAGER ? 'Team Lead' : 'Contributor',
    });
  }
  ...
  const projectStatuses = [ProjectStatus.ACTIVE, ProjectStatus.PLANNED, ProjectStatus.ON_HOLD, ProjectStatus.COMPLETED];
  const projectBatch: any[] = [];
  ...
  for (let i = 1; i <= 300; i++) {
    ...
    const health = status === ProjectStatus.ACTIVE ? Math.round((70 + Math.random() * 30) * 10) / 10 : 100.0;
  ...
  const taskStatuses = [TaskStatus.TODO, TaskStatus.IN_PROGRESS, TaskStatus.IN_REVIEW, TaskStatus.IN_QA, TaskStatus.DONE, TaskStatus.BACKLOG];
  ...
  for (let i = 1; i <= 5000; i++) {
    ...
    taskBatch.push({
      key: `TSK-${i}`,
      title: `Task #${i}: ${status === TaskStatus.DONE ? '[Completed]' : '[Active]'} Implement ${story ? story.title.substring(0, 35) : 'module feature'} - Module ${Math.ceil(i/100)}`,
      ...
      loggedHours: status === TaskStatus.DONE ? [2, 4, 6, 8, 12][i % 5] : 1.5,
